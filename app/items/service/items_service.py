...
class ItemsService:
    ...
    def rent_item(self, member_id: int, rent_data: RentRequest) -> dict:
        ...
        """
        # 1. category 문자열 → category_id 변환
        category = self.repo.get_category_by_name(rent_data.category_name)
        if not category:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"카테고리 '{rent_data.category_name}'을(를) 찾을 수 없습니다.",
            )
        ...
        if not available_item:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"'{rent_data.category_name}' 카테고리에 대여 가능한 물품이 없습니다.",
            )
